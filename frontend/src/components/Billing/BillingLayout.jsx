...
function BillingLayout({ categoriesData, getCategories, showNotification }) {
	...
	async function handleSubmit() {
		...
		let dataToSend = { ...customerDeatils, ...billInfo, items: items, invoice: invoice };
		try {
			const res = await axios.post(`${process.env.REACT_APP_BACKENDURI}/api/order`, dataToSend);
			if (res.data) {
				setItems([]);
				setBillInfo({ discount: 0, advance: 0, total: 0, note: "", deliveryDate: "" });
				setCustomerDeatils({
					name: "",
					phone: "",
				});
			...
			<div className="row">
				...
				<div className="col-lg-4">
					<div className="customer-info">
						...
						/>
						<TelInput label="Phone No:" name="phone" id="phone" placeholder="Phone No" onChange={validatePhone} value={customerDeatils.phone} variant="variant-1" required />
					...
					<div className="bill-info mt-4">
						...
						<div className="discount-input">
							<div className="d-flex align-items-center justify-content-between mt-2 total-flex">
								<p>Delivary Date:</p>
								<DateInput name="deliveryDate" value={billInfo.deliveryDate} onChange={(e) => setBillInfo((prev) => ({ ...prev, deliveryDate: e.target.value }))} />
							</div>
							<NumberInput
...
// 						<h4>Customer Deatils</h4>
// 						<hr />
// 						<TextInput label="Name:" name="name" id="customer-name" placeholder="customerName" onChange={handleChange} value={customerDeatils.name} variant="variant-1" required>
